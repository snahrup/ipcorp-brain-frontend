...
//
// Style-agnostic: this hook only reads geometry and writes transforms, so it
// came across essentially verbatim. Two changes from upstream:
//   1. the label geometry now comes from ./lifeline-metrics (upstream keeps it
//      in the labels component, which would drag a .tsx import in here);
//   2. the host-chrome alignment probe looks for [data-lifeline-align-start] /
//      [data-lifeline-align-end] instead of upstream's site-nav attributes. The
//      Workbench has no such nav, so it falls through to measuring its own box.

import { useCallback, useEffect, useLayoutEffect, useRef, useState } from "react";

import { LIFELINE_STICKY_LEFT, LIFELINE_STICKY_SHIELD_WIDTH } from "./lifeline-metrics";
...
/** Where the track starts when there is no host chrome to align with. */
...
/**
 * Embedded: once the rail bottoms out, the wheel keeps being swallowed until
 * the stream has been quiet this long. Trackpad inertia arrives as one unbroken
 * stream, so a fast flick that eats the last of the rail dies here instead of
 * spilling into the page behind it.
 */
const EMBED_BOUNDARY_QUIET_MS = 260;
/**
 * ...but never hold longer than this. Inertia decays; a finger or a wheel that
 * is still going does not, so a sustained scroll gets through.
 */
const EMBED_BOUNDARY_MAX_HOLD_MS = 900;

/**
 * How early the sweep is armed, relative to the module entering view. A couple
 * of hundred pixels means it is already moving by the time it is properly on
 * screen, rather than starting cold under the reader's eyes.
 */
...
function isInteractiveTarget(target: EventTarget | null) {
  // Pointer capture during drag retargets clicks to the section, so anything
  // clickable must opt out of drag-start here.
  return (
...
export function useLifelineScroll(markerCount: number, options: LifelineScrollOptions = {}) {
  ...
  const getLabelStageLeft = useCallback((translate: number) => {
    ...
    // Stage-relative: labels pin LIFELINE_STICKY_LEFT px inside the section's
    // own left edge, wherever the section sits on the page.
  ...
  const applyLabelSticky = useCallback(
    (translate: number) => {
      ...
      if (isSticky) {
        // Derived from the track's snapped offset so the two transforms cancel
        // to exactly LIFELINE_STICKY_LEFT — the pinned labels must not drift a
        // fraction against the snapped track.
  ...
  const updateFades = useCallback(() => {
    ...
    // All fade math is relative to the section's own box — the timeline may be
    // embedded anywhere, not pinned to the viewport.
    ...
    markerRefs.current.forEach((marker) => {
      ...
      // Fade a marker out only as scrubbing carries it left of where it rests
      // at translate 0 — the first markers naturally live inside the fade zone
      // and must not open dimmed.
  ...
  /**
   * Page mode or embedded? An explicit `mode` decides it outright. `"auto"`
   * measures: the timeline is the page only when it covers most of the viewport
   * *and* there is nothing behind it left to scroll. Both halves matter — a
   * full-bleed hero on a long page covers the viewport but must still hand the
   * wheel back at the ends.
   *
   * Cached for MODE_RESOLVE_STALE_MS because this reads layout and the wheel
   * path calls it. `force` is for measure passes, already doing layout work.
   */
  ...
  const measureLayout = useCallback(() => {
    ...
    const alignStart = document.querySelector("[data-lifeline-align-start]");
    const alignEnd = document.querySelector("[data-lifeline-align-end]");

    const startLeft = alignStart ? alignStart.getBoundingClientRect().left - stageRect.left : null;
    const endRight = alignEnd
      ? alignEnd.getBoundingClientRect().right - stageRect.left - NAV_HORIZONTAL_PADDING
      : null;

    /**
     * A full-page timeline always follows the host chrome. An embedded one
     * follows it only when the module actually spans it — a full-bleed module
     * lines its rail up with the chrome, while a timeline in a narrow card has
     * nothing to align to and measures itself instead.
     */
    const followChrome =
      !embed ||
      (startLeft !== null &&
        endRight !== null &&
        startLeft >= 0 &&
        endRight <= stageRect.width &&
        endRight > startLeft);

    startInset.current =
      followChrome && startLeft !== null ? startLeft : LIFELINE_DEFAULT_START_INSET;

    endInset.current =
      followChrome && endRight !== null
        ? endRight
  ...
  // Intentionally mount-once: this seeds the track position before first paint,
  // and re-running it on an identity change would re-seed mid-session. Resize
  // and marker changes go through the measure() path in the main effect below.
  ...
  useLayoutEffect(() => {
    ...
    if (!initialized.current) {
      // A skipped intro parks the rail where the intro would have settled it:
      // its end, the present.
  ...
  /**
   * A full-page timeline opens as the page opens, so its intro needs no cue. An
   * embedded one can be anywhere, including far below the fold — playing there
   * would spend the sweep on nobody. So it waits until the module is about to
   * come into view.
   */
  useEffect(() => {
    if (!isEmbed) return;
  ...
  useEffect(() => {
    ...
    // Embedded, hold the sweep until the module is in view.
    ...
    const step = (now: number) => {
      ...
      if (max <= 0) {
        ...
        // sweep. Waiting for one spun this loop forever and left the intro lock
        // on, so run the intro out where it already is — the markers and labels
        // still get their fade, there is just no journey.
  ...
  useEffect(() => {
    ...
    const startMomentum = () => {
      ...
      const step = (now: number) => {
        ...
        if (next <= 0 || next >= max) {
          // Coasting into an end counts as hitting it, so the next wheel event
          // starts its hold from the moment of contact.
    ...
    const scrub = (movement: number, target: number) => {
      ...
      // preference asks you to drop, and it is also the one thing that makes
      // the release decision non-deterministic.
    ...
    /**
     * Segments the wheel stream into gestures, before the section's own handler
     * sees the event — a capture listener on the window runs ahead of the
     * target. The section alone cannot tell a gesture that started on it from a
     * page-scroll gesture that merely arrived on it.
     */
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      // A sideways swipe is never a request to scroll the page, so it scrubs at
      // the rail ends too and never releases.
       ...
       *
       * Vertical is page scrolling: the intro leaves the rail at the present,
       * and scrolling down walks back through it. Horizontal is a drag by
       * another name, so it matches the pointer drag below: the rail follows
       * the fingers.
       */
      const movement = (horizontalIntent ? delta : -delta) * WHEEL_SPEED;

      if (!resolveMode()) {
        // Page mode: the timeline is the page and every wheel is ours.
      ...
      if (!horizontalIntent) {
        // A gesture already in flight when it reached us belongs to the page.
      ...
      if (target === translatePx.current && !horizontalIntent) {
        // At an end of the rail, still being pushed further out. Hold the wheel
        // until the stream goes quiet, then hand it to the page.
        const now = performance.now();
    ...
    // Arrow keys scrub only when the timeline effectively is the page, or when
    // focus is inside it. An embedded instance must not capture host keyboard
    // scrolling until the reader has tabbed into it.
    const ownsKeyboard = () => {
    ...
    window.addEventListener("wheel", onWheelStream, { passive: true, capture: true });
    ...
    // measureLayout's identity already tracks markerCount, so listing both
    // would tear the listeners down twice per marker-count change.
