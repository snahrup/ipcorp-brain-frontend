// Ported verbatim from evilrabbit/lifeline (MIT). Drives the narrow-viewport
// vertical layout, where the rail is a scroll position rather than a transform.
...
function getScrollParent(element: HTMLElement | null): HTMLElement | null {
  ...
  // a page-mode timeline in a page that just scrolls. Returning null here left
  // the whole rail invisible with no error.
...
interface LifelineVerticalScrollOptions {
   ...
   * would have settled it — the reader is arriving at a module in a page, not
   * returning to a timeline that already played.
   */
  isEmbed?: boolean;
  introLocked?: boolean;
...
export function useLifelineVerticalScroll(
  ...
  // Intentionally mount-once: seeds the scroll position before first paint;
  // re-running on an identity change would re-seed it mid-session.
  ...
  return { sectionRef, setEntryRef, isLayoutReady };
}
