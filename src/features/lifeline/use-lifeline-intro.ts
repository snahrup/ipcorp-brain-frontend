// Ported verbatim from evilrabbit/lifeline (MIT).
...
/**
 * Track length (px) the base rail duration was tuned for. Longer tracks slow
 * the sweep sublinearly so dense timelines stay readable, capped so a very
 * long one does not become a screensaver.
 */
...
/**
 * Keep fade stretching subtle — long fades lag behind the sweeping line and
 * read as out of sync.
 */
...
export function useLifelineIntro(markerWidths: number[]) {
  ...
  // Stretch each marker's fade with the sweep so dense timelines bloom in a
  // trailing wave instead of flickering past.
  ...
  const getMarkerFadeDuration = useCallback(
    (index: number) => Math.round(getTransitionMarkerFadeDuration(index) * fadeScale),
    [fadeScale]
  );
  ...
  useEffect(() => () => window.clearTimeout(introTimeoutRef.current), []);
